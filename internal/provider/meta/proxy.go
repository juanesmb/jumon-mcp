...
func (m *metaGateway) get(ctx context.Context, mcpTool, userID, path string, query map[string]string) (json.RawMessage, error) {
	resp, err := m.client.ProxyProvider(ctx, platformName, mcpTool, userID, "GET", path, query, nil, nil)
	return decodeMetaProxy(resp, err, m.client)
}
